...
public class SplitterEngine implements ITransformEngine, IAsyncTransformEngine {
    ...
    public void transform(URI original, JSONArray transform, URI transformed,
        ...
        try {
            job = new SplitJob(1);
            job.configure(original, transform, transformed, exporterOptions, null);
            job.submit(engine);
            job.join();
        } catch (Exception e) {
            if (job != null)
    ...
    @Override
    public Future<Object> transform(URI original, JSONArray transform, URI transformed, Properties exporterOptions, Callback callback) {
        try {
            SplitJob task = new SplitJob(1).configure(original, transform, transformed, exporterOptions, callback);
            task.submit(engine);
            return null;
        } catch (Exception e) {
            callback.failed(e);
